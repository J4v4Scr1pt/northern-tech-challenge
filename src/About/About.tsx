...
const About = () => {
	return (
		<Wrapper>
			...
			<span>
				<ul>
					<li>Workflow 🏃‍♂️</li> I usually start by reading the documentation/task then look at the
					...
					because of the timeframe for this challenge. (I do have a hobby project using Qwik 😁.)
					<li>Styling 💅</li>
					For the styling I choose Styled-Components because to me CSS-in-JS is the best DX. But I
					do think Tailwind or Vanilla-extract is really interesting and is the way forward.
					...
					things and I think when you get used to it you write faster and more stable code. It also
					helps with maintainability.
					<li>Testing 🧪</li>I have in the past used Jest as a test framework but recently started
			...
			<span>
				...
				during during CI(PR) run viteTest or if the team is up for it implement Husky and set up a
			...
			<span>
				...
				application is doing performance wise. Then if the result is bad, I would start tweaking the
				application to only serve the client as minimal javascript possible. I would start look into
				if there is any API calls that throttle the performance. If possible, I would look into if we
				...
				skeleton-loading to do perceived performance, that would make the performance feel better for
			...
			<span>
				...
				readability is as good as possible. I do think the code should speak for itself, but adding
				comments to unclear things. Implement more tests to ensure nothing is breaks with code
				changes. I do like the Vertical Slice Architecture (also known as Feature Architecture),
				because it makes it easy know where to go if you need to extend the features. And you separate
				the concerns to other features. You also make it easy to break out features to Micro-frontends
				or Island Architecture.
			</span>
			...
			<span>
				...
				includes mocked API calls. Remove the hard coded API keys and add them as environment
				variables in Vercel. I would also add more types to be able ro remove some use of the awful
				&apos;any&apos; word. Handle the inital animation of the slider button on Weather forecast tab
